...
const EMAIL_SUBJECTS: Record<string, string> = {
  signup: 'Xác nhận email cho Qilin',
  invite: 'Bạn được mời tham gia Qilin',
  magiclink: 'Link đăng nhập Qilin',
  recovery: 'Đặt lại mật khẩu Qilin',
  email_change: 'Xác nhận đổi email Qilin',
  reauthentication: 'Mã xác thực Qilin',
}
...
// Configuration
const SITE_NAME = "qilin"
const SENDER_DOMAIN = "notify.qilin.vn"
const ROOT_DOMAIN = "qilin.vn"
const FROM_DOMAIN = "qilin.vn" // Domain shown in From address (may be root or sender subdomain)
...
const SAMPLE_PROJECT_URL = "https://qilin.lovable.app"
...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  // Auto-confirm user for signup emails so they can log in immediately
  if (emailType === 'signup') {
    try {
      ...
      const targetUser = users.find((u: any) => u.email === payload.data.email)
      if (targetUser) {
        await adminClient.auth.admin.updateUserById(targetUser.id, { email_confirm: true })
        console.log('Auto-confirmed user', { email: payload.data.email, run_id })
      }
    } catch (err) {
      console.error('Auto-confirm failed', { error: err, run_id })
    }
  }
